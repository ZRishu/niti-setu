...
const Signup: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    // Crop validation
    if (!/^[a-zA-Z\s]+$/.test(formData.cropType)) {
      return "Crop name must only contain letters and spaces";
    }

    // Land holding validation: Digits and decimals only, non-negative
    if (!/^\d*\.?\d+$/.test(formData.landHolding) || parseFloat(formData.landHolding) < 0) {
      return "Land holding must be a positive number or decimal";
    }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // Proactive name, district, crop restriction: allow only letters and spaces
    if (['name', 'district', 'cropType'].includes(name)) {
      const onlyLetters = value.replace(/[^a-zA-Z\s]/g, '');
      setFormData({ ...formData, [name]: onlyLetters });
      return;
    }

    // Proactive land holding: allow only digits and single decimal
    if (name === 'landHolding') {
