    ...
    <div className="space-y-6 sm:space-y-8 animate-fade-in pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6">
        <MetricCard
          ...
          icon={<FileText className="h-4 w-4 sm:h-6 sm:w-6 text-blue-600" />}
          bgColor="bg-blue-50"
        />
        <MetricCard
        ...
        <MetricCard
          ...
          icon={<Clock className="h-4 w-4 sm:h-6 sm:w-6 text-purple-600" />}
          bgColor="bg-purple-50"
        />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Profile Summary - Hidden on Mobile, Shown on LG+ */}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base sm:text-xl font-bold text-slate-800">Smart Recommendations</h2>
            </div>
            <Link to="/search" className="text-[10px] sm:text-sm font-bold text-primary-600 hover:text-primary-700 flex items-center gap-1">
              Explore More <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4" />
            </Link>
          </div>
          ...
          {recsLoading ? (
                  ...
                  <div className="flex flex-col h-full justify-between">
                    <div>
                      <div className="flex items-start justify-between mb-3">
                        <div className="p-1.5 sm:p-2 bg-primary-50 rounded-lg text-primary-600 group-hover:bg-primary-600 group-hover:text-white transition-colors">
                          <FileText className="h-4 w-4 sm:h-5 sm:w-5" />
                        </div>
                        <ExternalLink className="h-3 w-3 sm:h-4 sm:w-4 text-slate-300 group-hover:text-primary-500 transition-colors" />
                      </div>
                      <h3 className="text-xs sm:text-base font-bold text-slate-900 group-hover:text-primary-700 transition-colors line-clamp-2">{scheme.name}</h3>
                    </div>
                    ...
                    <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-slate-50 flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[9px] sm:text-xs font-bold text-slate-600">
                          {scheme.benefits?.max_value_inr?.toLocaleString()}
                        </span>
                      </div>
                      <span className="px-1.5 py-0.5 sm:px-2 sm:py-1 bg-slate-100 text-slate-500 text-[8px] sm:text-[10px] font-bold rounded-md uppercase tracking-tight">
            ...
            <div className="bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 py-10 sm:py-16 text-center space-y-4 px-4">
              <div className="p-3 sm:p-4 bg-white rounded-full w-fit mx-auto shadow-sm">
                <Sparkles className="h-5 w-5 sm:h-8 sm:w-8 text-slate-300" />
              </div>
              <div>
                <p className="text-slate-500 font-bold text-xs sm:text-base">No personalized recommendations yet.</p>
                <p className="text-slate-400 text-[10px] sm:text-sm">Try updating your profile details to get AI suggestions.</p>
              </div>
              <Link 
                to="/profile"
                className="inline-flex items-center gap-2 px-5 py-2 sm:px-6 sm:py-2.5 bg-primary-600 text-white font-bold rounded-xl hover:bg-primary-700 transition-all shadow-lg shadow-primary-100 text-[10px] sm:text-sm"
              >
                Update Profile
              </Link>
            </div>
...
const MetricCard = ({ title, value, icon, bgColor }: any) => (
  <div className="bg-white p-4 sm:p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center gap-3 sm:gap-4">
    <div className={`p-2.5 sm:p-4 ${bgColor} rounded-xl sm:rounded-2xl`}>{icon}</div>
    <div>
      <p className="text-[9px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">{title}</p>
